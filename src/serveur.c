 ...
 * site web: http://bugsounet.fr
 *
 * Services pour serveur IRC. Supporté sur IrcDreams V.3
 *
...
int m_account(int parc, char **parv)
{
  ...
  if(parc < 3 || !(nick = num2nickinfo(parv[1])))
  {
    ...
    nick->user = NULL;
      ...
      Debug(W_DESYNCH|W_PROTO|W_WARN, "Erreur dans un TOKEN ACCOUNT de %s: arg=%d nick=%s", parv[0], parc, "NULL");
    return 0;
  }
  ...
  if((user = getuserinfo(parv[2])))
  {
    ...
    if(nick->user->level >0)
      osntc(nick, "\2You are now identified as Administrator. LEVEL: %d\2", nick->user->level);  
  }
  return 0;
}
...
int m_clearmode(int parc, char **parv)
{
  ...
  putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] = \2\0037CLEARMODE\2\3 %s by %s (%s)",
    os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], GetInfobyPrefix(parv[0]), parv[2]);
...
int m_mode(int parc, char **parv)
{
  ...
  if(*parv[1] == '#')
  {
    ...
    if(GetInfobyPrefix(parv[0]))
      putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] = \2\0037MODE\2\3 %s %s %sby %s",
        os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], parv[2], params, GetInfobyPrefix(parv[0]));
    else
      putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] = \2\0037MODE\2\3 %s %s %sby %s",
        os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], parv[2], params, num2servinfo(parv[0]));
  ...
  else if(nick)
  {
    ...
    if(GetInfobyPrefix(parv[0]))
      putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] = \2\0037MODE\2\3 %s %s by %s",
        os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], parv[2], GetInfobyPrefix(parv[0]));
    else
      putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] = \2\0037MODE\2\3 %s %s by %s",
        os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], parv[2], num2servinfo(parv[0]));
    ...
    if(*parv[2] == '+')
    {
      ...
      {
        if(*modes == 'a') {
          putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] ! \2\0037OPER\2\3 %s is now an IRC Administrator (+a)",
        ...
        if(*modes == 'o' && !(nick->flag & N_ADM))
        {
          putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] ! \2\0037OPER\2\3 %s is now an Irc Operator (+o)",
            os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1]);
          putserv("%s " TOKEN_WALLOPS " :%s is now an Irc Operator", os.num,parv[1]);
        }
      }
    }
...
int m_part(int parc, char **parv)
{
  ...
  putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] < \2\0037PART\2\3 %s by %s :%s",
    os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], nick, parc > 2 ? parv[2] : "Leaving");
...
int m_kick(int parc, char **parv)
{
  ...
  if(GetInfobyPrefix(parv[0]))
    putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] > \2\0037KICK\2\3 %s in %s by %s (%s)",
      os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, GetInfobyPrefix(parv[2]), parv[1], GetInfobyPrefix(parv[0]), parv[3]);
  else
    putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] > \2\0037KICK\2\3 %s in %s by %s (%s)",
      os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, GetInfobyPrefix(parv[2]), parv[1], num2servinfo(parv[0]), parv[3]);
...
int m_join(int parc, char **parv)
{
  ...
  putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] > \2\0037JOIN\2\3 %s by %s",
    os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], nick);
...
int m_create(int parc, char **parv)
{
  ...
  putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] > \2\0037CREATE\2\3 %s by %s",
    os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], nick);
...
int m_nick(int parc, char **parv)
{
  time_t tmt = CurrentTS;
  struct tm *ntime = localtime(&tmt);
  char *nick = parv[1], *ac = NULL, params[70], buf[NICKLEN + 1], hismask[NUHLEN + 1];
  int i=0, clonage=0, end = CurrentTS + 3600;
  aNick *who = NULL, *n;
  anUser *user;
  aBAD *bad;
  ...
  if(parc > 7) /*Nouveau nick*/
  {
    ...
    who = add_nickinfo(nick, parv[4], parv[5], parv[parc-3], parv[parc-2], serv, parv[parc-1], atol(parv[3]), *parv[6] == '+' ? parv[6] : NULL);

    if(!who) return 0; /* sauvetage temporaire..*/

    if(!burst)
      putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] > \2\0037NICK\2\3 %s \2\0037ADDRESS\2\3 %s@%s \2\0037SERVER\2\3 %s",
        os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, nick, parv[4], parv[5], serv);

    for(;i < NICKHASHSIZE;i++)
    {
      for(n = nick_tab[i];n;n = n->next)
      {
        if(tracehead && !burst) { // on ne sais jamais ...
          strcpy(hismask, who->nick);
          strcat(hismask, "!");
          strcat(hismask, who->ident);
          strcat(hismask, "@");
          strcat(hismask, who->host);
          for(;trace;trace = trace->next) {
            if(!match(trace->mask, hismask)) {
              oswallops("TRACE: %s se connecte [Mask: %s] (Raison: %s)", hismask, trace->mask, trace->raison);
                putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] > \2\0034TRACE\2\3 %s \2\0034MASK\2\3 %s \2\0034RAISON\2\3 %s",
                  os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, hismask, trace->mask, trace->raison);
            }
          }
        }

        if(!strcasecmp(n->host,who->host) && (strcasecmp(who->host,os.host)))
          clonage++;
      }
    }
    ...
    if((clonage > (clonemax -1)) && (!gettrusted(GetIP(who->base64)))) {
      strcpy(params, "*@");
  ...
  else if(parc <= 3) /* Changement de pseudo */
  {
    ...
    putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] = \2\0037NICK\2\3 from %s to %s",
      os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, who, nick);
  ...
  if(IsBadNick(nick) && !IsExNick(nick) && !IsOper(who) && (!who->user || !IsAdmin(who->user)))
  {
    ...
    putserv("%s "TOKEN_SVSNICK" %s :%s", bot.servnum, who->numeric, buf);
    return 0;
  }
  return 0;
}
...
int m_quit(int parc, char **parv)
{
  time_t tmt = CurrentTS;
  struct tm *ntime = localtime(&tmt);
  putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] < \2\0037QUIT\2\3 from %s : %s",
    os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, GetInfobyPrefix(parv[0]), parv[1]);
...
int m_kill(int parc, char **parv)
{
  ...
  if(!(nick = num2nickinfo(parv[1])) && parv[2][strlen(os.nick)] == ' ' && !strncmp(parv[2], os.nick, strlen(os.nick)))
    return 0;
  else if(!nick)
    return Debug(W_DESYNCH|W_WARN, "KILL de %s sur %s: aNick non trouvé!", parv[0], parv[1]);

  if(GetInfobyPrefix(parv[0]))
    putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] < \2\0037KILL\2\3 %s by %s :%s",
      os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, nick, GetInfobyPrefix(parv[0]), raison);
  else
    putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] < \2\0037KILL\2\3 %s by %s :%s",
      os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, nick, num2servinfo(parv[0]), raison);
...
int m_away(int parc, char **parv)
{
  ...
  if(!nick)
    return Debug(W_DESYNCH|W_WARN, "AWAY de %s: aNick non trouvé!", parv[0]);
  if(parc > 1 && *parv[1])
    nick->flag |= N_AWAY;
  else
    nick->flag &= ~N_AWAY;
...
int m_whois(int parc, char **parv)
{
  ...
  if(!strcasecmp(parv[2], os.nick))
    n = num2nickinfo(os.num);
  else if(!strcasecmp(parv[2], os.nick))
    n = num2nickinfo(os.num);
  else
    n = getnickbynick(parv[2]);
...
int m_topic(int parc, char **parv)
{
  ...
  if(parc < 2)
    return Debug(W_PROTO|W_WARN, "#arg invalide: TOPIC de %s: arg=%d", parv[0], parc);

  if(!burst)
  {
    if(num2servinfo(parv[0]))
      putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] = \2\0037TOPIC\2\3 %s %s \3by %s",
        os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], parv[parc-1], num2servinfo(parv[0]));
    else
      putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] = \2\0037TOPIC\2\3 %s %s \3by %s",
        os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1], parv[parc-1], num2nickinfo(parv[0]));
  }
  return 0;
}
...
int m_squit(int parc, char **parv)
{
  ...
  putserv("%s " TOKEN_PRIVMSG " %s :[%02d:%02d:%02d] < \2\0037SQUIT\2\3 by %s",
    os.num, bot.pchan, ntime->tm_hour, ntime->tm_min, ntime->tm_sec, parv[1]);

  if(parc < 2)
    return Debug(W_PROTO|W_WARN, "#arg invalide: SQUIT de %s: arg=%d", parv[0], parc);
...
int m_ping(int parc, char **parv)
{
  ...
  gettimeofday(&tv, NULL);
  /* AsLL */
  putserv("%s " TOKEN_PONG " %s %s %s %ld %ld.%ld",
    bot.servnum, bot.servnum, parv[0], parv[3], ((tv.tv_sec - atoi(parv[3])) * 1000 + (tv.tv_usec - atoi(theirttm)) / 1000), tv.tv_sec, tv.tv_usec);/* mon TS seconde.micro*/
  return 0;
}
...
static int exec_cmd(aHashCmd *cmd, unsigned int secure, aNick *nick, int parc, char **parv)
{
  ...
  if(SecureCmd(cmd) && !secure)
    osntc(nick, "La commande \2%s\2 demande des informations privées. Veuillez utiliser \2/%s %s %s\2.d",
  ...
  if(parc < cmd->args || (!parc && ChanCmd(cmd)))
    return syntax_cmd(nick, cmd);
  ...
  if(cmd->level)
  {
    ...
    if(AdmCmd(cmd) && nick->user->level < cmd->level)
      return osntc(nick, "Vous devez être Administrateur des Services pour utiliser cette commande.");

    if(ChanCmd(cmd) && !IsAdmin(nick->user))
    {
      if(!(acces = GetAccessIbyUserI(nick->user, chaninfo)))
        return osntc(nick, "Vous n'avez pas accès à %s.", parv[1]);

      if(cmd->level > acces->level)
        return osntc(nick, "Votre niveau sur %s est trop bas pour utiliser cette commande (Vous: \2%d\2, Commande: \2%d\2)",
...
int m_privmsg(int parc, char **parv)
{
  ...
  if(*parv[1] != '#') /* Message privé */
  {
    ...
    {
      if(*tmp != '\1') osntc(nptr, "La commande \2%s\2 n'existe pas.", tmp);
      return 0;
    }
