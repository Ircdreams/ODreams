 ...
 * $Id: config.c,v 1.8 2005/12/06 16:37:09 bugs Exp $
 */
...
struct conf_item conf_items[] = {
  /* uplink */
  {CONF_UPLINK, CONF_IP|CONF_TARRAY, CONF_MARRAY(bot.ip), "ip", "IP du HUB auquel le serveur doit se connecter", NULL},
  {CONF_UPLINK, CONF_TARRAY, CONF_MARRAY(bot.pass), "pass", "Pass du link", NULL},
  {CONF_UPLINK, CONF_TINT|CONF_PORT, &bot.port, 0, "port", "Port du HUB auquel le serveur doit se connecter", cf_validport},
  /* osbot */
  ...
  {CONF_OSBOT, CONF_TARRAY, CONF_MARRAY(os.mode), "modes", "UserModes que doit porter le OS à la connexion (+ok normalement)", NULL},
  {CONF_OSBOT, CONF_TARRAY, CONF_MARRAY(bot.pchan), "chan", "Salon de log où est envoyée l'activité du OS", cf_validchan},
  /* myserv */
  {CONF_MYSERV, CONF_TARRAY, CONF_MARRAY(bot.server), "server", "Nom littéral du serveur", cf_validserv},
  {CONF_MYSERV, CONF_TARRAY, CONF_MARRAY(bot.name), "infos", "Infos du serveur, visible dans le /links", NULL},
  {CONF_MYSERV, CONF_TPRIV|CONF_TINT, NULL, 0, "numeric", "Numeric P10 du serveur (en chiffre)", cf_validnum},
  {CONF_MYSERV, CONF_TPTR, &network, 0, "network", "Nom du réseau", NULL},
  {CONF_MYSERV, CONF_TPTR, &quit_msg, 0, "quit_msg", "Message envoyé lors de la maintenance des services", NULL},
  /* misc */
  {CONF_MISC, CONF_TPTR, &pasdeperm, 0, "pas_de_perm", "message envoyé à un user utilisant une commande nécessitant d'être logué.", NULL},
  {CONF_MISC, CONF_TPTR, &defraison, 0, "defraison", "Raison par default des Kicks / Glines / Kills / Shuns", NULL},
    /* Operserv */
  {CONF_MISC, CONF_TINT, &clonemax, 0, "maxclones", "Nombre de clones maximum autorisé", NULL},
  {CONF_MISC, CONF_TPTR, &blockmsg, 0, "blockmsg", "Message du G-line en cas d'utilisation de la fonction Block", NULL},
  {CONF_MISC, CONF_TPTR, &shunmsg, 0, "shunmsg", "Message du Shun en cas d'utilisation de la fonction Silence", NULL},
  {CONF_MISC, CONF_TPTR, &chnick, 0, "chnick", "Préfix mis devant les chiffres aléatoires des pseudos donnés par le robot lors de la prise d'un pseudo interdit", NULL},
};
